...
wide_data <- test_data %>%
  pivot_wider(
    ...
    names_prefix = "t") %>%
  left_join(
    test_data %>% dplyr::select(patient_id, age, sex, bmi) %>% slice(1),
    by = "patient_id"
  )

model_poisson <- flexmix(
  cbind(t0, t1, t2, t3, t4, t5, t6, t7, t8, t9,
        t10, t11, t12, t13, t14, t15, t16, t17, t18, t19) ~ 1,
  k = 4,
  model = FLXMRglm(family = "poisson"),
  control = list(minprior = 0.05, iter.max = 500, verbose = 1),
  data = wide_data
)
```
...
model_type <- "zip"  # <--- CHANGE THIS TO SWITCH MODELS
# ========================================
...
#' Fit GBTM with specified distribution - CORRECTED FOR LONGITUDINAL DATA
#' 
#' @param data Data frame in long format (multiple rows per patient)
...
fit_gbtm <- function(data, 
  ...
  # CRITICAL FIX: For longitudinal data, flexmix needs to know which rows belong together
  # We do NOT use the | operator for this - instead we use the cluster argument
  
  # Ensure data is sorted by patient
  data <- data %>% arrange(!!sym(id_var))
  
  # Create a cluster variable (factor version of patient ID)
  data$cluster_id <- factor(data[[id_var]])
  
  cat("\nFitting", k, "class", toupper(model_type), "GBTM model...\n")
  cat("Formula:", deparse(formula), "\n")
  cat("Clustering by:", id_var, "\n")
  cat("Total observations:", nrow(data), "\n")
  cat("Unique patients:", length(unique(data[[id_var]])), "\n")
  
  # Fit model with proper longitudinal specification
  set.seed(1008)  # For reproducibility
  
  fit <- tryCatch({
    flexmix(
      formula = formula,  # Just the trajectory formula, no | operator
      data = data,
      k = k,
      cluster = data$cluster_id,  # THIS IS THE KEY: tells flexmix about repeated measures
      model = model_driver,
      control = list(
  ...
  if (!is.null(fit)) {
    ...
    # Print class sizes
    cat("\nPreliminary class sizes:\n")
    print(table(clusters(fit)))
  }
...
#' Extract posterior probabilities and class assignments - CORRECTED
#' 
...
extract_class_assignments <- function(model, data) {
  ...
  # Get posterior probabilities (one row per PATIENT, not per observation)
  post_probs <- posterior(model)
  
  # Get class assignments (which class has highest probability for each PATIENT)
  class_assign <- clusters(model)
  
  # CRITICAL: Get unique patient IDs in the SAME ORDER as they appear in the model
  # Since we used cluster = data$cluster_id, the order matches unique(data$cluster_id)
  patient_ids <- unique(data$patient_id)
  
  # Verify dimensions match
  cat("\nDimension check:\n")
  cat("  Posterior probabilities: ", nrow(post_probs), "rows\n")
  cat("  Class assignments: ", length(class_assign), "elements\n")
  cat("  Unique patients: ", length(patient_ids), "patients\n")
  
  if (nrow(post_probs) != length(patient_ids)) {
    stop(paste("ERROR: Posterior has", nrow(post_probs), 
               "rows but there are", length(patient_ids), "unique patients.",
               "\nThis suggests the model didn't properly handle longitudinal structure."))
  }
  
  if (length(class_assign) != length(patient_ids)) {
    stop(paste("ERROR: Class assignments has", length(class_assign), 
               "elements but there are", length(patient_ids), "unique patients."))
  }
  
  # Convert posterior probabilities to data frame
  post_probs_df <- as.data.frame(post_probs)
  ...
  assignments <- cbind(assignments, post_probs_df)
  ...
  if (length(prob_cols) == 0) {
    ...
    prob_cols <- names(post_probs_df)
    warning("Using default column names for posterior probabilities")
  }
  
  cat("Found", length(prob_cols), "probability columns\n")
  
  # Calculate max posterior probability for each patient
  if (length(prob_cols) > 0) {
    assignments$max_posterior_prob <- apply(
      assignments[, prob_cols, drop = FALSE], 
      1, 
      function(x) {
        if (all(is.na(x))) {
          return(NA)
    ...
    stop("Could not find probability columns in posterior output")
  }
  
  # Check for problems
  n_missing <- sum(is.na(assignments$max_posterior_prob))
  if (n_missing > 0) {
    warning(paste(n_missing, "patients have missing posterior probabilities"))
  }
  
  # Summary
  cat("\n=== Class Assignment Summary ===\n")
  print(table(assignments$predicted_class))
  cat("\nPosterior probability summary:\n")
  print(summary(assignments$max_posterior_prob))
